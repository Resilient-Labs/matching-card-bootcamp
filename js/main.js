...
function checkCards() {
  ...
  //this will keep the front faces showing up!
    ...
    //SET THE STATE OF THE CARDS TO BE PERMANENTLY UP!!
  }
  clickedBox = [];
...
function winningBoard() {
  //if each of the boxes have a front-face, then
  let facesToArray = Array.from(faces);
  console.log(facesToArray);
    let checkFaces = facesToArray.every((card) => {
  ...
  if (checkFaces === true){
    ...
    document.querySelector("h2").innerText="Keep Trying"
  }
  console.log(checkFaces);
}
