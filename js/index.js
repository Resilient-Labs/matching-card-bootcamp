...
let firstCard = null;
//null reps the intentional absence of any object value
...
function flipCard(event) {
  ...
  if (firstCard == null) {
    ...
    //will get here only if first card is set to sth. first card is not equal to clickedCrad
    //let's check to see if first card matches flipped card
    setTimeout(checkMatch, 700, firstCard, clickedCard);
    //firstCard and clickCard are passsed into checkMAtch as arg on both checkMAtch and setTimeout (checkMatch)
...
function checkWin() {
  ...
  if (
    ...
    document.querySelector("h3").innerText = "Congrats — You Have Won!";
  }
}

function resetGame() {
  document.querySelector("h3").innerText ="Match The Cards & Win!"
  let picArray = [
    "images/travora.jpg",
    "images/kelly.jpg",
    "images/dennis.jpg",
  ...
  Array.from(document.querySelectorAll(".card img:not(.backCard)")).forEach(
    (img) => (img.src = "")
  );
  for (let i = 0; i < 10; i++) {
    let emptyCards = document.querySelectorAll('img[src=""]');
    // NodeList objects -> returned by document.querySelectorAll().
